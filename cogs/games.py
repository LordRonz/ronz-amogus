...
class Games(commands.Cog):
    ...
    async def trivia(self, ctx):
        ...
        try:
            ...
            await ctx.send('You did not answer in time, what the fuck??')
            return

        if len(ans:=msg.content.lower()) != 1 or not 0 <= (i:=ord(ans) - 97) < 5:
            await ctx.send(f'no dumbass, the correct answer was `{triv.correct_answer}`')
            return

        if triv.answers[i] == triv.correct_answer:
            await ctx.send('You got that answer correct genius')
